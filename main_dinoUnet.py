# main_dino_2d_testvol.py
# Same structure as your ResNet/UNet/UNeXt 2D main, but:
#  - Model is imported from your already-implemented file: Dino_MultiLabel.py
...
# Project path setup
# ============================================================
THIS_DIR = os.path.dirname(os.path.abspath(__file__))          # .../my_project/Ibrahim_Dino_Unet
...
from Ibrahim.metrics.Multilabel_metrics import (
    init_running_metrics,
    update_running_metrics,
    finalize_metrics,
)

# ============================================================
# IMPORT YOUR DINO MODEL
# IMPORTANT: adjust the import line to match YOUR project layout.
# ============================================================
from Ibrahim.networks.dinov3_multilabel import DINOv3_MultiLabel
...
def _prep_2d_batch(images, labels, device):
    ...
    DINO backbone expects 3 channels, so if channel=1 => repeat to 3.
    """
    if images.dim() == 3:
...
def evaluate_on_test_2d(model, test_loader, num_classes, device, ce_loss, dice_loss):
    ...
    with torch.no_grad():
        for samples in tqdm(test_loader, total=len(test_loader), desc="TEST_2D(test_vol->slices)"):
            ...
            logits = model(imgs)  # expected logits (B,C,H,W)
...
def get_argparser():
    ...
    p.add_argument("--exp_name", type=str, default="Synapse_DINOv3_2D_testvol")
    ...
    # DINO backbone / weights
    p.add_argument("--model_name", type=str, default="vit_base_patch16_dinov3.lvd1689m")
    p.add_argument("--weights_path", type=str,
                   default="/data3/nkozah/my_project/Ibrahim_Dino_Unet/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth")
...
def main():
    ...
    for epoch in range(opts.NB_EPOCH):
        ...
        for i, samples in enumerate(tqdm(train_loader, desc="TRAIN_2D", total=len(train_loader))):
            ...
            # compute metrics sparsely (keeps train faster)
