...
# -------------------------
# ADDED (minimal) to adapt to new dataset_synapse.py signature/paths
# -------------------------
def _resolve_synapse_root_dir(root_path_train: str) -> str:
    """
    dataset_synapse.py expects root_dir containing:
      - train_npz/
      - test_vol_h5/
      - lists_Synapse/
    Your opts.root_path_train may point to .../train_npz, so we normalize it.
    """
    p = os.path.abspath(root_path_train)
    if os.path.basename(p.rstrip(os.sep)) == "train_npz":
        p = os.path.dirname(p)
    return p

def _resolve_list_dir_name(list_dir_path: str) -> str:
    """
    dataset_synapse.py wants list_dir_name (folder name inside root_dir).
    Your opts.list_dir is a full path like .../lists_Synapse.
    """
    return os.path.basename(os.path.abspath(list_dir_path).rstrip(os.sep))
...
def main():
    ...
    # -------------------------
    # CHANGED (only this block) to match new dataset_synapse.py
    # -------------------------
    synapse_root_dir = _resolve_synapse_root_dir(opts.root_path_train)
    list_dir_name = _resolve_list_dir_name(opts.list_dir)

    train_dataset = Synapse_dataset(
        root_dir=synapse_root_dir,
        split="train",
        transform=tr_transform,
        list_dir_name=list_dir_name
    )
    # -------------------------
