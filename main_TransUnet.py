# main_transunet_2d_testvol.py
# Same structure as your UNet/UNeXt/ResNet 2D main, but:
#  - Model = TransUNet (your already-implemented VisionTransformer)
#  - Test split = "test_vol" evaluated as 2D slices extracted from volumes
#  - CSV includes macro + per-class metrics (Dice/Jaccard/HD95) for train and test
#  - Adds EarlyStopping (with patience + min_delta)
...
# ============================================================
# IMPORT YOUR TRANSUNET (already implemented elsewhere)
# IMPORTANT: adjust the import line to match YOUR project layout.
# Example: if your file is Ibrahim/networks/A1220_TransUnet_vit_seg_modeling.py
# ============================================================
from Ibrahim.networks.TransUnet_MultiLabel import VisionTransformer, CONFIGS
...
def _prep_2d_batch(images, labels, device):
    """
    Accepts:
      - images (B,H,W)  -> becomes (B,1,H,W)
      - images (B,1,H,W)
      - images (B,3,H,W)
    TransUNet forward already repeats 1->3, but we keep this consistent/safe.
    """
    if images.dim() == 3:
...
def _per_class_to_arrays(per_class, C):
    """
    Robust flatten:
    - if per_class is dict {class_id: (dice,jac,hd95)} or {class_id: {...}}
    - if per_class is list/tuple length C with either (dice,jac,hd95)
    Returns 3 lists of length C.
    """
    dice = [np.nan] * C
    jac = [np.nan] * C
    hd95 = [np.nan] * C
...
class EarlyStopping:
    """
    mode:
      - "max": metric improves when it increases (e.g., macro_dice)
      - "min": metric improves when it decreases (e.g., loss)
    """
    ...
    def step(self, current):
        if current is None or (isinstance(current, float) and np.isnan(current)):
            # treat invalid metric as no improvement
            self.bad_epochs += 1
...
# ============================================================
# Test-vol -> slice dataset (same logic)
# ============================================================
...
def evaluate_on_test_2d(model, test_loader, num_classes, device, ce_loss, dice_loss):
    ...
    with torch.no_grad():
        for samples in tqdm(test_loader, total=len(test_loader), desc="TEST_2D(test_vol->slices)"):
            ...
            logits = model(imgs)  # logits (B,C,H,W)
...
def get_argparser():
    ...
    p.add_argument("--exp_name", type=str, default="Synapse_TransUNet_2D_testvol")
    ...
    # TransUNet config
    p.add_argument("--vit_name", type=str, default="R50-ViT-B_16",
                   choices=list(CONFIGS.keys()))
    p.add_argument("--vit_pretrained", type=str, default="",  # optional path to pretrained npz/pth if you load it elsewhere
                   help="(Optional) path hint; not used unless you implement loading in your project.")
...
def main():
    ...
    # Model (TransUNet)
    # -------------------------
    # config object from your CONFIGS dict (ml-collections style)
    config = CONFIGS[opts.vit_name]
    # --- fix invalid grid for given img_size ---
    img = int(opts.img_size)
    if config.patches.get("grid") is not None:
        gs = config.patches["grid"]  # tuple/list like (gH, gW)
        base_h = img // 16
        base_w = img // 16

        # if grid is too large -> division becomes zero -> crash
        if base_h < gs[0] or base_w < gs[1]:
            new_g = (min(gs[0], base_h), min(gs[1], base_w))
            # avoid zeros entirely
            new_g = (max(1, new_g[0]), max(1, new_g[1]))
            print(f"[WARN] patches.grid {gs} too large for img_size={img}. "
                f"Clamping grid -> {new_g} (base={base_h}x{base_w}).")
            config.patches["grid"] = new_g

    # IMPORTANT: ensure these fields exist in your config
    # many TransUNet configs use: config.n_classes, config.n_skip, config.skip_channels, config.decoder_channels, ...
    # We'll set n_classes to match Synapse classes.
    try:
        config.n_classes = C
    except Exception:
        # fallback if config is dict-like only
        config["n_classes"] = C

    # expected: logits (B,C,H,W)
    model = VisionTransformer(config, img_size=opts.img_size, num_classes=C, zero_head=False, vis=False).to(device)
    ...
    for epoch in range(opts.NB_EPOCH):
        ...
        # best model logic (based on TEST macro_dice or TRAIN loss, like your template)
        if opts.save_best_on == "macro_dice":
            score = te_macro_dice
