# main_deeplabv3plus_2d_testvol.py
...
# CHANGE THIS IMPORT
from Ibrahim.networks.DeepLab_MultiLabel import DeepLabHeadV3Plus

from torchvision.models import resnet50, resnet101, resnet18, resnet34
from Ibrahim.networks.DeepLab_utils import IntermediateLayerGetter


class ResNetFeatureExtractor(nn.Module):
    def __init__(self, name="resnet50", pretrained=True, replace_stride_with_dilation=(False, True, True)):
        super().__init__()
        if name == "resnet50":
            backbone = resnet50(pretrained=pretrained, replace_stride_with_dilation=replace_stride_with_dilation)
            low_ch, out_ch = 256, 2048
        elif name == "resnet101":
            backbone = resnet101(pretrained=pretrained, replace_stride_with_dilation=replace_stride_with_dilation)
            low_ch, out_ch = 256, 2048
        elif name == "resnet34":
            backbone = resnet34(pretrained=pretrained, replace_stride_with_dilation=replace_stride_with_dilation)
            low_ch, out_ch = 64, 512
        elif name == "resnet18":
            backbone = resnet18(pretrained=pretrained, replace_stride_with_dilation=replace_stride_with_dilation)
            low_ch, out_ch = 64, 512
        else:
            raise ValueError(f"Unknown backbone: {name}")

        backbone.fc = nn.Identity()
        self.body = IntermediateLayerGetter(backbone, return_layers={"layer1": "low_level", "layer4": "out"})
        self.low_level_channels = low_ch
...
class DeepLabV3Plus(nn.Module):
    ...
    def forward(self, x):
        ...
        return nn.functional.interpolate(logits, size=sz, mode="bilinear", align_corners=False)
...
def evaluate_on_test_2d(model, test_loader, num_classes, device, ce_loss, dice_loss):
    ...
    return per_class, macro, (loss_sum / max(1, n_batches), ce_sum / max(1, n_batches), dice_sum / max(1, n_batches))


def _parse_bool_csv(s, n=3):
    parts = [p.strip() for p in s.split(",") if p.strip() != ""]
    if len(parts) != n:
        raise ValueError(f"Expected {n} values, got: {s}")
...
def get_argparser():
    ...
    p.add_argument("--exp_name", type=str, default="Synapse_DeepLabV3Plus_2D_testvol")
    ...
    p.add_argument("--backbone", type=str, default="resnet50", choices=["resnet18", "resnet34", "resnet50", "resnet101"])
    p.add_argument("--backbone_pretrained", type=int, default=1, choices=[0, 1])
    p.add_argument("--dilate", type=str, default="0,1,1")
    p.add_argument("--aspp_dilate", type=str, default="12,24,36")
...
def main():
    ...
    print("[OK] building model...", flush=True)
    replace_stride_with_dilation = _parse_bool_csv(opts.dilate, n=3)
    aspp_rates = _parse_int_csv(opts.aspp_dilate)

    backbone = ResNetFeatureExtractor(
        name=opts.backbone,
        pretrained=bool(opts.backbone_pretrained),
        replace_stride_with_dilation=replace_stride_with_dilation,
    )
    head = DeepLabHeadV3Plus(
        in_channels=backbone.out_channels,
        low_level_channels=backbone.low_level_channels,
        num_class=C,
        aspp_dilate=aspp_rates
    )
    model = DeepLabV3Plus(backbone, head).to(device)
