# main_hiformer_2d_testvol.py
#
# SAME structure as your main_efficientunet_2d_testvol.py, but:
#   - Model = HiFormer_MultiLabel (logits: (B,C,H,W))
#   - Synapse multi-class training: CrossEntropy + MultiClassDiceLoss
#   - Test split = "test_vol" evaluated as 2D slices extracted from volumes
#
# Run example:
#   python main_hiformer_2d_testvol.py \
#     --hiformer_variant S \
#     --root_dir /data3/nkozah/my_project/Data/synapse \
#     --out_root /data3/nkozah/my_project/Ibrahim/experiments \
#     --exp_name Synapse_HiFormerS_2D_testvol \
#     --img_size 224 --num_classes 9 --batch_size 4 --test_batch_size 8 \
#     --in_chans 3 --repeat_to_3 1
#
# IMPORTANT:
# - If your data is 1-channel CT, your dataset returns (B,H,W) then we add channel and optionally repeat to 3.
# - HiFormer pretrained Swin tiny is typically for 3-channel -> keep repeat_to_3=1 unless you changed encoder.
...
from Ibrahim.networks.HiFormer_MultiLabel import HiFormer_MultiLabel

# Your config factories (the code you pasted)
from Ibrahim.networks.HiFormer_configs import (
    get_hiformer_s_configs,
    get_hiformer_b_configs,
    get_hiformer_l_configs,
)


def build_hiformer_cfg(variant: str):
    v = str(variant).lower().strip()
    if v in ("s", "small"):
        return get_hiformer_s_configs()
    if v in ("b", "base"):
        return get_hiformer_b_configs()
    if v in ("l", "large"):
        return get_hiformer_l_configs()
...
def _prep_2d_batch(images, labels, device, repeat_to_3: bool):
    if images.dim() == 3:
        images = images.unsqueeze(1)
    imgs = images.to(device, dtype=torch.float32, non_blocking=True)
    if repeat_to_3 and imgs.shape[1] == 1:
...
def evaluate_on_test_2d(model, test_loader, num_classes, device, ce_loss, dice_loss, repeat_to_3: bool):
    model.eval()
    ...
    with torch.no_grad():
        for samples in tqdm(test_loader, total=len(test_loader), desc="TEST"):
            imgs, gts = _prep_2d_batch(samples["image"], samples["label"], device, repeat_to_3=repeat_to_3)
            out = model(imgs)
...
def get_argparser():
    ...
    p.add_argument("--exp_name", type=str, default="Synapse_HiFormer_2D_testvol")
    ...
    # HiFormer
    p.add_argument("--hiformer_variant", type=str, default="S", choices=["S", "B", "L"])
    p.add_argument("--in_chans", type=int, default=3)
    p.add_argument("--repeat_to_3", type=int, default=1, choices=[0, 1])
    p.add_argument("--return_probs", type=int, default=0, choices=[0, 1])
    p.add_argument("--multilabel", type=int, default=0, choices=[0, 1])
...
def _build_model(opts) -> nn.Module:
    cfg = build_hiformer_cfg(opts.hiformer_variant)
    # keep config consistent with args
    cfg.image_size = int(opts.img_size)
    cfg.num_classes = int(opts.num_classes)

    model = HiFormer_MultiLabel(
        config=cfg,
        img_size=opts.img_size,
        in_chans=opts.in_chans,
        n_classes=opts.num_classes,
        return_probs=bool(opts.return_probs),
        multilabel=bool(opts.multilabel),
    )
    return model
...
def main():
    ...
    print("[OK] building model (HiFormer_MultiLabel)...", flush=True)
    model = _build_model(opts).to(device)
    print("[OK] model ready", flush=True)

    # optional sanity check
    try:
        with torch.no_grad():
            ch = 3 if opts.repeat_to_3 else opts.in_chans
            x = torch.randn(2, ch, opts.img_size, opts.img_size, device=device)
            y = model(x)
        y0 = y[0] if isinstance(y, (tuple, list)) else y
        print(f"[OK] sanity forward: {tuple(y0.shape)} (expected: (B,{C},H,W))", flush=True)
    ...
    # Synapse default: multi-class (softmax + CE)
    if opts.multilabel == 1:
        raise RuntimeError("Synapse pipeline here is multi-class (targets B,H,W). Use --multilabel 0.")
    ...
    for epoch in range(opts.NB_EPOCH):
        ...
        for i, samples in enumerate(tqdm(train_loader, desc="TRAIN", total=len(train_loader))):
            imgs, gts = _prep_2d_batch(
                samples["image"], samples["label"], device,
                repeat_to_3=bool(opts.repeat_to_3),
            )
